        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 md:p-12">
          ...
          <div className="text-justify space-y-6">
            <p className="text-slate-700 leading-relaxed max-w-3xl">
              Explore our comprehensive range of specialized registrations and
              licenses tailored to keep your business compliant and competitive.
            </p>
          </div>
          ...
          <div className="mt-16">
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
              {REGISTRATIONS.map((reg, index) => {
                ...
                const hrefPath =
                  reg.title === "Startup India"
                    ? "/startup-india"
                    : reg.title === "ICEGATE"
                      ? "/icegate"
                      : reg.title === "Import Export Code"
                        ? "/import-export-code"
                        : reg.title === "PF Registration"
                          ? "/pf-registration"
                          : reg.title === "ESI Registration"
                            ? "/esi-registration"
                            : reg.title === "Udyam (MSME)"
                              ? "/udyam-registration"
                              : reg.title === "Trade Mark"
                                ? "/trademark-registration"
                                : reg.title === "Legal Entity Identification"
                                  ? "/lei-registration"
                                  : reg.title === "Professional Tax"
                                    ? "/professional-tax-registration"
                                    : reg.title === "Shop & Establishment"
                                      ? "/shop-establishment-registration"
                                      : reg.title === "Trade License"
                                        ? "/trade-license-registration"
                                        : reg.title === "FSSAI License"
